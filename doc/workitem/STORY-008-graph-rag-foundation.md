...
- **Context pack budgets**: implementation plan baseline — regression mode max 12k tokens, diagnostic mode max 20k tokens; lower-priority sections shrink first and blocking mismatches are never truncated
...
2. Context pack token count stays within mode-specific envelope limits (regression 12k, diagnostic 20k); blocking mismatches are never truncated even when at capacity (lower-priority sections shrink first).
...
- `TCN-S6-001` — Query against non-ready index returns retryable `INDEX_NOT_READY` error
- `TCN-S6-002` — Invalid filter field is rejected (no silent fallback to broad query)
- `TCE-S6-001` — Oversized retrieval result set is truncated deterministically with overflow metadata logged
...
- [ ] Context pack respects mode-specific budgets (regression 12k / diagnostic 20k)
...
- [ ] `TC-S6-001` through `TC-S6-004`, `TCN-S6-001`, `TCN-S6-002`, `TCE-S6-001` pass in `integration` profile
